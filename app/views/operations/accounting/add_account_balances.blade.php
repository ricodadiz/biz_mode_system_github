...
<div class="row">
                        <div class="col-sm-6 col-md-3">
                            <a class="block block-link-hover3 text-center" href="{{URL::to('accounting/'.$company->id.'/balance_list')}}">
                                ...
                                <div class="block-content block-content-full block-content-mini bg-gray-lighter text-success font-w600">Back to Starting Balance</div>
                            </a>
                        </div>
</div> 
...
<div class="block block-bordered">
                        <div class="block-header bg-gray-lighter">
                            ...
                            <h3 class="block-title">Add Starting Balance</h3>
                        </div>
                        <div class="block-content">
                            <form class="form-horizontal push-10-t push-10" action="{{URL::to('accounting/{id}/add_account_balances')}}" method="post" id="form_id">
                                <div class="row">
                                    <div class="col-sm-7">
                                        <div class="form-group">
                                            <div class="col-xs-12">
                                                <label for="mega-firstname">Balance Name</label>
                                                <input class="form-control input-lg" type="text" id="account_balance_name" name="account_balance_name" placeholder="---------------">
                                            </div>
                                            <div class="col-xs-12">
...
{{--                                 COMPUTE THE TOTAL DEBIT AND CREDIT --}}
    ...
    var submit=0;
        $("#newline").click(function(){
                            $(".new").append('<div class="form-group">'+'<div class="col-xs-12">'+'<label for="mega-firstname">Account</label>'+'<select class="form-control input-lg" id="contact1-subject" name="account_name[]" size="1">'+'<option disabled selected></option>' +'@foreach($category as $c)'+'<option style="font-weight: bolder" disabled>{{$c}}</option>'+'@foreach($account_name as $an)'+'@if($c===$an->category)'+'<option>&nbsp;&nbsp;&nbsp;{{$an->name}}</option>'+'@endif'+'@endforeach'+'@endforeach'+'</select>'+'</div>');
